...
export default function Stock() {
  ...
  const precios = [
    ...
    { label: 'Mas de 20M', min: 20000000, max: Infinity },
  ]
    ...
    <section className='min-h-screen pt-28 pb-20 px-6'>
      <div className='max-w-7xl mx-auto'>
        ...
        <div className='flex flex-wrap gap-3 mb-8 pb-8 border-b border-zinc-800'>
          ...
          {(marcaFiltro !== 'Todas' || precioFiltro !== 'Todos' || transmisionFiltro !== 'Todas') && (<button onClick={() => { setMarcaFiltro('Todas'); setPrecioFiltro('Todos'); setTransmisionFiltro('Todas') }} className='text-orange-500 border border-orange-500/30 hover:border-orange-500 text-xs tracking-widest px-4 py-2.5 transition-all'>LIMPIAR X</button>)}
        </div>
        {autosFiltrados.length > 0 ? (
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
            {autosFiltrados.map((auto) => (<CardAuto key={auto.id} auto={auto} />))}
          </div>
        ) : (
